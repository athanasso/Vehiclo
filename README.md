...
- **Fuel Logs** — Track fill-ups with auto fuel economy calculation. Automatically displays official gas station branch logos globally. Flexible entry modes (Total Cost vs Volume) with full inline edit/delete and OCR receipt scanning.
- **Trip Logs & Auto-Tracking** — Log trips manually. Alternatively, enable Native Android Activity Recognition background tracking: a themed auto-detection modal proactively intercepts pending drives and asks to import them upon returning to the app, complete with a drawn SVG route map of your GPS points and exact timestamps. Route maps are saved and viewable in your trip history alongside calculated Uber/Taxi transport savings.
